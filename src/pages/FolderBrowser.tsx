...
import {
  Folder, FileText, Upload, Plus, ChevronRight, Home,
  MoreHorizontal, Download, Trash2, Edit3, Star, FileArchive, X, Tag as TagIcon, Eye, FolderUp, Bookmark, MessageSquare
} from "lucide-react";
import { useParams, useNavigate, Link } from "react-router-dom";
import { useFolderContents, useCreateFolder, useSoftDeleteFolder, useRenameFolder } from "@/hooks/useFolders";
...
export default function FolderBrowser() {
  ...
  function handleCreateFolder(e: React.FormEvent) {
    ...
    createFolder.mutate(
      { name: newFolderName.trim(), parentFolderId: currentFolderId },
  ...
  return (
    <AppLayout
      title="Folders & Files"
      subtitle="Shared storage — every team member can edit anything here."
      actions={
        <>
          <input
            ref={fileInputRef}
    ...
    >
      ...
      <div className="mb-5 flex items-center gap-1.5 text-sm text-text-secondary">
        <Link to="/files" className="flex items-center gap-1 hover:text-text-primary">
          <Home className="h-3.5 w-3.5" /> Root
        </Link>
        {path?.map((p) => (
          <span key={p.id} className="flex items-center gap-1.5">
            <ChevronRight className="h-3.5 w-3.5" />
            <Link to={`/folders/${p.id}`} className="hover:text-text-primary">{p.name}</Link>
          </span>
        ))}
      </div>
      ...
      >
        ...
        {!isLoading && data?.folders.length === 0 && data?.files.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="font-medium text-text-secondary">This folder is empty</p>
